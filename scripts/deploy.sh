...
current_function="countWords"
current_build="${current_function}_stage"
cd src/functions/countWords
...
then
    ...
    functionArn=$(aws lambda create-function --function-name "$current_build" --runtime nodejs16.x --role arn:aws:iam::$AWS_ACCOUNT_ID:role/lambda-basic-role --handler lambdaCtx.handler --zip-file fileb://./"${current_build}.zip" | jq -r '.FunctionArn')
    if [ -z "$functionArn" ]
    then
